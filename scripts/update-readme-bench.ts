// Parses a bench-output.txt file produced by `bun run bench` and rewrites the
// auto-generated benchmark block in README.md between <!-- BENCH:START --> and
// <!-- BENCH:END --> markers.
...
interface EngineResults {
  // case opsPerSec map, e.g. "rq.plain" -> 19710000
  ops: Record<string, number>;
  // pair speedup map, e.g. "rq.plain vs URL.plain" -> 17.85
...
function main(): void {
  ...
  // Hot-path throughput line: rq.plain ops/sec per engine.
  const hotPath = ENGINE_ORDER.map(
    (e) => `~${fmtMillions(results[e].ops["rq.plain"] ?? 0)}`
  ).join(" / ");
  const throughputLine = `Absolute throughput on the hot path: **${hotPath} ops/s** for \`readQueryParam\` (V8 / SpiderMonkey / JSC) on the benchmark host.`;
