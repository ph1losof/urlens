...
describe("view()", () => {
  test("returns the structural UrlView API", () => {
    const result = view("https://example.com");
    expect(typeof result.pathname).toBe("function");
    expect(typeof result.queryParam).toBe("function");
  });
...
describe("UrlView.port()", () => {
  ...
  test("returns null for malformed (non-digit) ports", () => {
    ...
    expect(view("https://example.com:80x0/p").port()).toBeNull();
  });
...
describe("UrlView.queryParam()", () => {
  ...
  test("caches query encoding after a miss", () => {
    const plain = view("https://x/?a=1&b=2");
    expect(plain.queryParam("missing")).toBeNull();
    expect(plain.queryParam("other")).toBeNull();

    const encoded = view("https://x/?a=1&encoded%20key=value");
    expect(encoded.queryParam("missing")).toBeNull();
    expect(encoded.queryParam("encoded key")).toBe("value");
  });

  test("does not repeat equals-sign searches after a bare field", () => {
    expect(view("https://x/?a&b&c&%ZZ=value").queryParam("%ZZ")).toBe("value");
    expect(view("https://x/?a&b&c").queryParam("%ZZ")).toBeNull();
  });

  test("ignores empty segments for an empty parameter name", () => {
    expect(view("https://x/?&&=value").queryParam("")).toBe("value");
    expect(view("https://x/?&&").queryParam("")).toBeNull();
  });

  test("does not mistake field delimiters for parameter names", () => {
    const current = view("https://x/?==wrong&&%3D=equals&%26=ampersand");
    expect(current.queryParam("=")).toBe("equals");
    expect(current.queryParam("&")).toBe("ampersand");
  });
});
...
describe("UrlView.queryParams()", () => {
  ...
  test("reuses a scalar lookup for duplicate keys", () => {
    expect(view("https://x/?q=value").queryParams(["q", "q"] as const).q).toBe(
      "value"
    );
  });
  ...
  test("returns one property for duplicate keys without a query", () => {
    const out = view("https://x/p").queryParams(["a", "a"] as const);
    expect(out).toEqual({ a: null });
  });

  test("uses an own property for the __proto__ key", () => {
    const out = view("https://x/?__proto__=value&q=ok").queryParams([
      "__proto__",
      "q",
    ] as const);
    expect(Object.hasOwn(out, "__proto__")).toBe(true);
    expect(Object.getOwnPropertyDescriptor(out, "__proto__")?.value).toBe(
      "value"
    );
    expect(out.q).toBe("ok");
  });

  test("uses an own __proto__ property in generic batches", () => {
    const present = view("https://x/?__proto__=value&q=ok").queryParams([
      "__proto__",
      "q",
      "missing",
    ] as const);
    expect(Object.getOwnPropertyDescriptor(present, "__proto__")?.value).toBe(
      "value"
    );
    expect(present.missing).toBeNull();

    const absent = view("https://x/?q=ok").queryParams([
      "__proto__",
      "q",
      "missing",
    ] as const);
    expect(
      Object.getOwnPropertyDescriptor(absent, "__proto__")?.value
    ).toBeNull();
  });

  test("scans three or more requested keys in one pass", () => {
    const out = view("https://x/?a=1&b=2&c=3").queryParams([
      "c",
      "missing",
      "a",
    ] as const);
    expect(out).toEqual({ c: "3", missing: null, a: "1" });
  });
  ...
  test("reads a key at the start of a long query", () => {
...
describe("UrlView.hasQueryParam()", () => {
  ...
  test("does not mistake field delimiters for parameter names", () => {
    const current = view("https://x/?==wrong&&%3D=equals&%26=ampersand");
    expect(current.hasQueryParam("=")).toBe(true);
    expect(current.hasQueryParam("&")).toBe(true);
    expect(view("https://x/?==wrong&&").hasQueryParam("=")).toBe(false);
  });
});
...
describe("UrlView.queryParamEquals()", () => {
  ...
  test("does not mistake field delimiters for parameter names", () => {
    const current = view("https://x/?==wrong&&%3D=equals&%26=ampersand");
    expect(current.queryParamEquals("=", "equals")).toBe(true);
    expect(current.queryParamEquals("&", "ampersand")).toBe(true);
    expect(view("https://x/?==wrong&&").queryParamEquals("=", "wrong")).toBe(
      false
    );
  });
});
...
describe("UrlView WHATWG-decoded key matching", () => {
  ...
  test("queryParam rejects ambiguous-user-key false positives", () => {
    // The URL key decodes to "a b", not the literal key "a%20b".
