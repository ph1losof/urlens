...
describe("readQueryParam", () => {
  ...
  test("ignores empty segments when reading an empty parameter name", () => {
    expect(readQueryParam("https://example.com/?", "")).toBeNull();
    expect(readQueryParam("https://example.com/?&", "")).toBeNull();
    expect(readQueryParam("https://example.com/?&=value", "")).toBe("value");
    expect(readQueryParam("https://example.com/?&&=value", "")).toBe("value");
  });

  test("does not mistake field delimiters for parameter names", () => {
    const url = "https://example.com/?==wrong&&%3D=equals&%26=ampersand";
    expect(readQueryParam(url, "=")).toBe("equals");
    expect(readQueryParam(url, "&")).toBe("ampersand");
  });
...
describe("readQueryParams", () => {
  ...
  test("does not treat empty segments as empty parameter names in batches", () => {
    expect(
      readQueryParams("https://example.com/?&&=value&a=1", ["", "a"])
    ).toEqual(["value", "1"]);
    expect(
      readQueryParams("https://example.com/?&&=value&a=1", ["", "a", "b"])
    ).toEqual(["value", "1", null]);
  });
...
describe("setQueryParams", () => {
  ...
  test("skips empty segments before replacing an empty parameter name", () => {
    expect(
      setQueryParams("https://example.com/?&&=old&a=1", {
        "": "new",
        a: null,
      })
    ).toBe("https://example.com/?=new");
...
describe("hasQueryParam", () => {
  ...
  test("does not mistake field delimiters for parameter names", () => {
    const url = "https://x.test/?==wrong&&%3D=equals&%26=ampersand";
    expect(hasQueryParam(url, "=")).toBe(true);
    expect(hasQueryParam(url, "&")).toBe(true);
    expect(hasQueryParam("https://x.test/?==wrong&&", "=")).toBe(false);
    expect(hasQueryParam("https://x.test/?==wrong&&", "&")).toBe(false);
  });
...
describe("queryParamEquals", () => {
  ...
  test("does not mistake field delimiters for parameter names", () => {
    const url = "https://x.test/?==wrong&&%3D=equals&%26=ampersand";
    expect(queryParamEquals(url, "=", "equals")).toBe(true);
    expect(queryParamEquals(url, "&", "ampersand")).toBe(true);
    expect(queryParamEquals("https://x.test/?==wrong&&", "=", "wrong")).toBe(
      false
    );
  });
  ...
  test("byte-walk: WHATWG-conformant U+FFFD emission for malformed UTF-8", () => {
    // Match TextDecoder replacement behavior for malformed UTF-8 sequences.
...
describe("removeQueryParam", () => {
  ...
  test("preserves miss input and normalizes a leading empty field on removal", () => {
    const url = "https://x.test/?&a=1";
    expect(removeQueryParam(url, "missing")).toBe(url);
    expect(removeQueryParam(url, "a")).toBe("https://x.test/");
  });
...
describe("removeQueryParams", () => {
  ...
  test("skips empty segments before removing an empty parameter name", () => {
    expect(
      removeQueryParams("https://x.test/?&&=value&a=1", ["", "missing"])
    ).toBe("https://x.test/?a=1");
  });

  test("keeps existing empty-field normalization after a lazy prefix", () => {
    expect(
      removeQueryParams("https://x.test/?&&a=1&&b=2&", ["a", "missing"])
    ).toBe("https://x.test/?b=2");
  });
...
describe("WHATWG-decoded key matching", () => {
  ...
  describe("setQueryParam — decoded keys", () => {
    test("replaces an encoded key using WHATWG serialization", () => {
      expect(
    ...
    test("does not match ambiguous user key against literal byte form", () => {
      // Append the encoded caller key because the URL key decodes differently.
