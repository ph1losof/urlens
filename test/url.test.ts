...
describe("readOrigin", () => {
  ...
  test("handles authorities longer than the packed-offset fast path", () => {
    ...
    expect(readHost(`https://user@${hostname}/path`).length).toBe(
      hostname.length
    );
  });
  ...
  test("uses the last literal @ in the authority as the userinfo terminator", () => {
    // WHATWG requires literal '@' in userinfo to be encoded.
...
describe("rawOriginsEqual", () => {
  test("returns true for identical origins", () => {
    expect(rawOriginsEqual("https://x.test/a", "https://x.test/b")).toBe(true);
    expect(
      rawOriginsEqual("https://x.test:8080/a", "https://x.test:8080/b")
    ).toBe(true);
  });

  test("returns false for different schemes", () => {
    expect(rawOriginsEqual("https://x.test/", "http://x.test/")).toBe(false);
    expect(rawOriginsEqual("http://x.test/", "htxp://x.test/")).toBe(false);
  });

  test("returns false for different hostnames", () => {
    expect(rawOriginsEqual("https://a.test/", "https://b.test/")).toBe(false);
    expect(rawOriginsEqual("https://short.test/", "https://longer.test/")).toBe(
      false
    );
  });

  test("infers implicit ports for special schemes", () => {
    expect(rawOriginsEqual("https://x.test:443/", "https://x.test/")).toBe(
      true
    );
    expect(rawOriginsEqual("http://x.test:80/", "http://x.test/")).toBe(true);
    expect(rawOriginsEqual("ws://x.test:80/", "ws://x.test/")).toBe(true);
    expect(rawOriginsEqual("wss://x.test:443/", "wss://x.test/")).toBe(true);
    expect(rawOriginsEqual("ftp://x.test:21/", "ftp://x.test/")).toBe(true);
  });

  test("returns false for genuinely different ports", () => {
    expect(rawOriginsEqual("https://x.test:8080/", "https://x.test/")).toBe(
      false
    );
    expect(rawOriginsEqual("https://x.test:80/", "https://x.test/")).toBe(
      false
    );
  });

  test("does not infer implicit ports for non-special schemes", () => {
    expect(rawOriginsEqual("custom://x.test:9/", "custom://x.test/")).toBe(
      false
    );
    expect(rawOriginsEqual("custom://x.test:9/", "custom://x.test:9/")).toBe(
      true
    );
    expect(rawOriginsEqual("custom://x.test/", "custom://x.test/")).toBe(true);
    expect(rawOriginsEqual("custom://x.test/", "custom://x.test:9/")).toBe(
      false
    );
  });

  test("returns false for malformed explicit ports", () => {
    expect(rawOriginsEqual("https://x.test:bad/", "https://x.test/")).toBe(
      false
    );
    expect(rawOriginsEqual("https://x.test/", "https://x.test:bad/")).toBe(
      false
    );
  });

  test("scheme comparison is case-insensitive", () => {
    expect(rawOriginsEqual("HTTPS://x.test/", "https://x.test/")).toBe(true);
    expect(rawOriginsEqual("HttP://x.test/", "http://x.test/")).toBe(true);
  });

  test("hostname comparison is case-insensitive", () => {
    expect(
      rawOriginsEqual("https://EXAMPLE.com/", "https://example.com/")
    ).toBe(true);
    expect(
      rawOriginsEqual("https://Example.COM/", "https://example.com/")
    ).toBe(true);
  ...
  test("IPv6 host case-insensitive comparison", () => {
    expect(
      rawOriginsEqual("http://[2001:DB8::1]/", "http://[2001:db8::1]/")
    ).toBe(true);
  });

  test("strips userinfo from both sides", () => {
    expect(rawOriginsEqual("https://u:p@x.test/a", "https://x.test/b")).toBe(
      true
    );
    expect(rawOriginsEqual("https://u1@x.test/a", "https://u2@x.test/b")).toBe(
      true
    );
  });

  test("handles IPv6 hosts", () => {
    expect(rawOriginsEqual("http://[::1]:8080/a", "http://[::1]:8080/b")).toBe(
      true
    );
    expect(rawOriginsEqual("http://[::1]/a", "http://[::1]/b")).toBe(true);
    expect(rawOriginsEqual("http://[::1]:8080/a", "http://[::1]/b")).toBe(
      false
    );
  });

  test("returns false when either URL has no scheme", () => {
    expect(rawOriginsEqual("/path", "https://x.test/")).toBe(false);
    expect(rawOriginsEqual("https://x.test/", "")).toBe(false);
  });

  test("is not a security-boundary comparison for unnormalized input", () => {
    const unnormalized = String.raw`https://evil.test\@trusted.test/`;
    expect(rawOriginsEqual(unnormalized, "https://trusted.test/")).toBe(true);
    expect(new URL(unnormalized).origin).toBe("https://evil.test");
  });
});
